    ...
    with patch.dict(os.environ, {"DEFAULT_ADMIN_ID": ""}):
        with patch("app_fastapi.main.init_db", return_value=None):
            with patch("app_fastapi.main.logger") as mock_logger:
                await startup()
                mock_logger.warning.assert_called_with("DEFAULT_ADMIN_ID not found in .env")


@pytest.mark.asyncio
async def test_startup_failure_handling():
    with patch("app_fastapi.main.init_db", side_effect=Exception("DB initialization failed")):
        with patch("app_fastapi.main.logger") as mock_logger:
            with pytest.raises(Exception):
                await startup()

            mock_logger.critical.assert_called_with("Application startup failed: DB initialization failed", exc_info=True)
