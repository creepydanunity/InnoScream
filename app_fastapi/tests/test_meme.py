...
def fixed_env(monkeypatch):
    """Set fixed environment variables and random.choice for tests."""
...
class AsyncClientMock:
    ...
    async def post(self, url, data):
        """Simulate httpx.AsyncClient.post with controlled response."""
...
async def test_generate_meme_url_success(monkeypatch):
    """Test successful meme URL generation."""
    payload = {
        "success": True,
    ...
    monkeypatch.setattr(
        httpx,
        "AsyncClient",
        lambda: AsyncClientMock(response_json=payload)
    )
...
async def test_generate_meme_url_exception(monkeypatch, caplog):
    """Test exception handling during meme generation."""
    monkeypatch.setattr(
        httpx,
        "AsyncClient",
