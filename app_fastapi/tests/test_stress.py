...
from app_fastapi.tests.conftest import TestingSessionLocal, engine, Base
from app_fastapi.api.endpoints import get_session
...
async def test_get_weekly_stress_graph_all():
    ...
    async with TestingSessionLocal() as session:
        for i in range(7):
            ...
            scream = Scream(content=f"s{i}", user_hash=f"u{i}", timestamp=ts)
            session.add(scream)
    ...
    expected_labels = [(week_ago + timedelta(days=i)).strftime("%a") for i in range(7)]
