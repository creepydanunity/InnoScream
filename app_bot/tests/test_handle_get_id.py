...
async def test_handle_my_id_success():
    ...
    with mock.patch("app_bot.handlers.getIdHandler.get_my_id", return_value={"user_id": "123456789"}):
...
async def test_handle_my_id_failure():
    ...
    with mock.patch("app_bot.handlers.getIdHandler.get_my_id", side_effect=Exception("API error")):
        await handle_my_id(mock_msg)
