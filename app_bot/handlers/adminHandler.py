...
async def handle_delete(msg: types.Message, state: FSMContext):
    """Handle /delete command to start moderation session."""
    screams = []
    try:
    ...
    try:
        ...
        message_text = (
            f"🧠 Scream {scream_index + 1} out of {len(screams)}:\n\n"
            f"📌Scream_id: {current['scream_id']}\n\n"
            f"📝Content:\n{current['content']}"
        )
        await msg.answer(
            text=message_text,
...
async def process_callback_button_back(
    callback_query: CallbackQuery,
    state: FSMContext
):
    """Handle back navigation in moderation feed."""
...
async def handle_create_admin(msg: types.Message):
    """Handle /create_admin command to assign admin rights."""
    try:
        args = msg.text.split()
        user_id = str(msg.from_user.id)
        if len(args) != 2:
            logger.warning(
                f"Invalid create_admin command format from user {user_id}"
            )
        ...
        logger.info(
            f"Admin creation attempt by {user_id} for {user_id_to_admin}"
        )
        ...
        if e.response.status_code == 403:
            ...
            logger.error(
                f"HTTP error in create_admin: {str(e)}",
                exc_info=True
            )
            await msg.answer("Server error occurred.")
    except Exception as e:
        logger.error(
            f"Failed to assign admin rights: {str(e)}",
            exc_info=True
        )
...
async def process_callback_button_delete(
    callback_query: CallbackQuery,
    state: FSMContext
):
    """Handle delete action for a scream in moderation feed."""
    try:
        data = await state.get_data()
        screams = data["screams"]
        index = data["index"]
        ...
        if result['status'] == "deleted":
            ...
            message_text = (
                f"🧠 Scream {index + 1} out of {len(screams)}:\n\n"
                f"📌Scream_id: {next_scream['scream_id']}\n\n"
                f"📝Content:\n{next_scream['content']}"
            )
        ...
        logger.error(
            f"Failed to process delete action: {str(e)}",
            exc_info=True
        )
...
async def process_callback_button_confirm(
    callback_query: CallbackQuery,
    state: FSMContext
):
    """Handle confirm action for a scream in moderation feed."""
    try:
        data = await state.get_data()
        screams = data["screams"]
        index = data["index"]
        ...
        user_id = str(callback_query.from_user.id)
        logger.info(
            f"Admin {user_id} confirming scream {current['scream_id']}"
            )
...
async def process_callback_button_next(
    callback_query: CallbackQuery,
    state: FSMContext
):
    """Handle next navigation in moderation feed."""
    try:
        data = await state.get_data()
        scream_index = (data["index"] + 1) % len(data["screams"])
        screams = data["screams"]
        ...
        logger.error(
            f"Failed to navigate next: {str(e)}",
            exc_info=True
        )
...
async def process_callback_button_exit(
    callback_query: CallbackQuery,
    state: FSMContext
):
    """Handle exit action from moderation session."""
    try:
        user_id = str(callback_query.from_user.id)
        ...
        logger.error(
            f"Failed to exit moderation: {str(e)}",
            exc_info=True
        )
