...
def test_react_not_found(client, missing_id):
    """
    Reacting to a non-existent scream ID must return 404 and the correct error message.
    """
    resp = client.post(
        "/react",
...
def test_react_success_and_duplicate(client):
    """
    A first valid reaction should succeed (200 OK), and a duplicate reaction by the same user should be rejected (409).
    """
    async def seed_and_test():
        """
        Seed a scream in the database, perform a reaction, verify persistence, then test duplicate rejection.
        """
